...
export const TOOLS: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  {
    ...
    function: {
      ...
      description:
        "Take a viewport screenshot of the currently loaded page or a given URL and return it as a base64 JPEG. Use sparingly for one-off visual/layout checks; prefer inspect_responsive_rendering for page-by-page visual QA because it returns structured rendering evidence and streams screenshots to the report gallery.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "run_lighthouse",
      description:
        "Run a Lighthouse lab audit against a URL using a fresh headless Chrome instance (Playwright Chromium or system Chrome via chrome-launcher) and return compact but detailed evidence. Returns url/finalUrl/formFactor/requestedFormFactor/configPreset/fetchTime, category scores for performance/accessibility/best-practices/seo (0-1 or null), core Web Vitals metrics (first-contentful-paint, largest-contentful-paint, total-blocking-time, cumulative-layout-shift, speed-index, interactive with displayValue/score/numericValue when available), categoryAuditRefs, up to 20 opportunities, up to 20 diagnostics, failedAudits/manualAudits/notApplicableAudits samples, passedAuditCount, runWarnings, and errors if the run could not be completed. Lighthouse audit entries include id/title/description/displayValue/score/scoreDisplayMode/numericValue/numericUnit/detailsType, potential savings such as wastedMs/wastedBytes when available, and a compact sample of details.items for URLs/resources/nodes. Use opportunities, diagnostics, and failedAudits as evidence for the report's Lighthouse and improvement recommendations sections. The `formFactor` parameter selects the Lighthouse preset: 'mobile' (default) or 'desktop' (uses lighthouseModule.desktopConfig when available, with an explicit desktop settings fallback so the run is still a real desktop audit). Mobile and desktop runs may be requested separately to compare. The deterministic preflight already includes both form factors. NOTE: This tool is slow (typically 30-90s per run) and uses its own Chrome instance.",
      parameters: {
        type: "object",
        properties: {
          url: { type: "string" },
